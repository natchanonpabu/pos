...
```
ชั้น 1: Global Config (~/.claude/)
  └── CLAUDE.md  →  @skill-1-personal.md
        │
        │  "โปรเจกต์ในโฟลเดอร์ ~/Documents → ใช้ skill-1 (personal)"
        ↓
...
```
1. ~/.claude/CLAUDE.md                     ← Global entry point
   └── ~/.claude/skills/skill-1-personal.md  ← โหลดเพราะ project อยู่ใน ~/Documents
...
Claude มี skill profiles ระดับ global อยู่ที่ `~/.claude/skills/` สำหรับโปรเจกต์นี้จะโหลด **skill-1-personal** อัตโนมัติ (เพราะ project อยู่ใน `~/Documents`) ซึ่งกำหนด core style เช่น TypeScript strict, single responsibility functions, และ comment เฉพาะ WHY ไม่ต้องสนใจรายละเอียด global config — ส่วนที่เกี่ยวข้องกับโปรเจกต์นี้ถูก override และเพิ่มเติมใน project files ด้านล่างทั้งหมดแล้ว
...
[โหลด context อัตโนมัติ]
  CLAUDE.md chain + skill-1 + memory
      │
      ▼
[วิเคราะห์ task]
  เล็ก/ชัดเจน? → ทำเลย
